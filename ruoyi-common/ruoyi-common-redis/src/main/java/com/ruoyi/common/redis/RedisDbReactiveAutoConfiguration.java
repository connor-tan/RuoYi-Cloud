...
public class RedisDbReactiveAutoConfiguration implements InitializingBean, DisposableBean {
    ...
    public ReactiveRedisTemplate<Object, Object> reactiveRedisTemplate(ReactiveRedisConnectionFactory reactiveRedisConnectionFactory, ResourceLoader resourceLoader) {
        ...
        for (Map.Entry<String, RedisProperties> entry : redisDbProperties.getConfig().entrySet()) {
            ...
            if (defaultConfig.equals(key)) {
                SpringUtils.registerSingleton(join(key, REACTIVE_REDIS_TEMPLATE), reactiveRedisTemplate);
            } else {
                ReactiveRedisConnectionFactory connectionFactory = SpringUtils.getBean(join(key, REDIS_CONNECTION_FACTORY), ReactiveRedisConnectionFactory.class);
                SpringUtils.registerSingleton(join(key, REACTIVE_REDIS_TEMPLATE), new ReactiveRedisTemplate<>(connectionFactory, serializationContext));
            }
        }
    ...
    public ReactiveStringRedisTemplate reactiveStringRedisTemplate(ReactiveRedisConnectionFactory reactiveRedisConnectionFactory) {
        ...
        for (Map.Entry<String, RedisProperties> entry : redisDbProperties.getConfig().entrySet()) {
            String key = entry.getKey();
            if (defaultConfig.equals(key)) {
                SpringUtils.registerSingleton(join(key, REACTIVE_STRING_REDIS_TEMPLATE), reactiveStringRedisTemplate);
            } else {
                ReactiveRedisConnectionFactory connectionFactory = SpringUtils.getBean(join(key, REDIS_CONNECTION_FACTORY), ReactiveRedisConnectionFactory.class);
                SpringUtils.registerSingleton(join(key, REACTIVE_STRING_REDIS_TEMPLATE), new ReactiveStringRedisTemplate(connectionFactory));
            }
        }
