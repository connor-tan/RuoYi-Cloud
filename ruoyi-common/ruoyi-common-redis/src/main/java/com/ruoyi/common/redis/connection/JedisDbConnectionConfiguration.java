package com.ruoyi.common.redis.connection;


import com.ruoyi.common.core.utils.SpringUtils;
import com.ruoyi.common.redis.RedisDbProperties;
import com.ruoyi.common.redis.RedisProperties;
...
@ConditionalOnProperty(
        name = {"spring.tower.redis.client-type"},
        havingValue = "jedis",
        matchIfMissing = true
)
public class JedisDbConnectionConfiguration extends RedisDbConnectionConfiguration {
    ...
    JedisConnectionFactory redisConnectionFactoryVirtualThreads(
        ...
        for (Map.Entry<String, RedisProperties> entry : this.getProperties().getConfig().entrySet()) {
            ...
            if (this.getProperties().getDefaultConfig().equals(key)) {
                ...
                JedisConnectionFactory connectionFactory = SpringUtils.getBean(join(key, REDIS_CONNECTION_FACTORY), JedisConnectionFactory.class);
    ...
    private JedisConnectionFactory createJedisConnectionFactory(ObjectProvider<JedisClientConfigurationBuilderCustomizer> builderCustomizers, RedisConnectionDetails connectionDetails) {
        ...
        for (Map.Entry<String, RedisProperties> entry : this.getProperties().getConfig().entrySet()) {
            ...
            RedisConnectionDetails redisConnectionDetails = defaultConfig.equals(key) ? connectionDetails : SpringUtils.getBean(join(key, REDIS_CONNECT_DETAILS), RedisConnectionDetails.class);
            ...
            SpringUtils.registerSingleton(join(key, REDIS_CONNECTION_FACTORY), jedisConnectionFactory);
        }
