    ...
    LettuceConnectionFactory redisConnectionFactoryVirtualThreads(
        ...
        for (Map.Entry<String, RedisProperties> entry : this.getProperties().getConfig().entrySet()) {
            ...
            if (this.getProperties().getDefaultConfig().equals(key)) {
                ...
                LettuceConnectionFactory connectionFactory = SpringUtils.getBean(join(key, REDIS_CONNECTION_FACTORY), LettuceConnectionFactory.class);
    ...
    private LettuceConnectionFactory createConnectionFactory(ObjectProvider<LettuceClientConfigurationBuilderCustomizer> builderCustomizers,
        ...
        for (Map.Entry<String, RedisProperties> entry : this.getProperties().getConfig().entrySet()) {
            ...
            if (defaultConfig.equals(key)) {
                ...
                RedisConnectionDetails redisConnectionDetails = SpringUtils.getBean(join(key, REDIS_CONNECT_DETAILS), RedisConnectionDetails.class);
            ...
            SpringUtils.registerSingleton(join(key, REDIS_CONNECTION_FACTORY), connectionFactory);
        }
